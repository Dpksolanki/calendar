...
const BookingPage = async ({
  ...
  const { date: selectedDateString, time: selectedTimeString } = await searchParams;
  const selectedDate = new Date(selectedDateString || new Date().toISOString());

  const eventType = await getData(username, eventName);
  // console.log(123, eventType, 23)c
        ...
        <Card className="max-w-[600px]">
          <CardContent className="p-5 grid md:grid-cols-[1fr,auto,1fr] gap-4">
            ...
            >
              ...
              <input type="hidden" name="fromTime" value={selectedTimeString} />
              <input type="hidden" name="eventDate" value={selectedDateString} />
              <input
                type="hidden"
                name="meetingLength"
